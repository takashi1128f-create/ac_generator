...
window.renderTyreUI = function() {
	...
	const tyreTabs = [
		{ id: 'BASIC', label: 'BASIC', keys: ['VERSION','NAME', 'SHORT_NAME', 'WIDTH', 'RADIUS', 'RIM_RADIUS', 'ANGULAR_INERTIA', 'DAMP', 'RATE', 'FLEX', 'XMU', 'FZ0'] },
		{ id: 'LON_GRIP', label: 'LON (縦)', keys: ['DX_REF', 'DX0', 'DX1', 'LS_EXPX', 'CX_MULT', 'RADIUS_ANGULAR_K', 'BRAKE_DX_MOD'] },
		{ id: 'LAT_GRIP', label: 'LAT (横)', keys: ['DY_REF', 'DY0', 'DY1', 'LS_EXPY', 'CAMBER_GAIN', 'DCAMBER_0', 'DCAMBER_1', 'FRICTION_LIMIT_ANGLE'] },
		{ id: 'SLIDE', label: 'SLIDE (過渡)', keys: ['FLEX_GAIN', 'FALLOFF_LEVEL', 'FALLOFF_SPEED', 'SPEED_SENSITIVITY', 'RELAXATION_LENGTH', 'ROLLING_RESISTANCE_0', 'ROLLING_RESISTANCE_1', 'ROLLING_RESISTANCE_SLIP'] },
		{ id: 'PRESSURE', label: 'PRESSURE', keys: ['PRESSURE_STATIC', 'PRESSURE_SPRING_GAIN', 'PRESSURE_FLEX_GAIN', 'PRESSURE_RR_GAIN', 'PRESSURE_D_GAIN', 'PRESSURE_IDEAL'] },
		{ id: 'THERMAL', label: 'THERMAL', keys: ['PERFORMANCE_CURVE', 'SURFACE_TRANSFER', 'PATCH_TRANSFER', 'CORE_TRANSFER', 'INTERNAL_CORE_TRANSFER', 'FRICTION_K', 'ROLLING_K', 'SURFACE_ROLLING_K', 'COOL_FACTOR'] },
		{ id: 'WEAR', label: 'WEAR (摩耗)', keys: ['WEAR_CURVE', 'GRAIN_GAMMA', 'GRAIN_GAIN', 'BLISTER_GAMMA', 'BLISTER_GAIN'] }
	];
		...
		<div class="drivetrain-editor-layout">
			<article class="tyres_select-outer_box">
				...
				<aside class="gear-sidebar">
					...
					<button class="add-set-btn" onclick="window.addTyreCompound()">+ 追加</button>
				</aside>
			</article>
	...
	// 🌟 修正：FRONT用・REAR用と2つあった入れ物を1つに統一
	const inputContainer = document.getElementById('tyre-side-inputs');

	// ★ 項目名（key）を見て、自動で正しいセクションを判定するロジック
	const thermalKeys = [
		...
		'GRAIN_GAMMA', 'GRAIN_GAIN', 'BLISTER_GAMMA', 'BLISTER_GAIN'
	];

	activeTabDef.keys.forEach(key => {
		// そのキーが THERMAL 系かどうかを判定
		const isThermalKey = thermalKeys.includes(key);

		// 🌟 修正：現在選択されている側（FRONTかREAR）のセクション名を決定
		const secName = isThermalKey ? `THERMAL_${window.activeTyreSide}` : window.activeTyreSide;

		// 🌟 修正：現在の側のデータだけを取得して描画する
		const secData = currentCompound.data[secName] || {};
...
window.addTyreInputField = function(parent, compoundObj, sectionName, key, val) {
	...
	// 🌟 修正：CSSクラス（groupClass, tyre-indicator-icons など）を割り当てる
	itemDiv.innerHTML = `
		<div class="input-unit${colorClass}${groupClass}">
			<label>${key}</label>
			<div class="input-with-range tyre-input-wrapper">
				<input type="${isNum ? 'number' : 'text'}" class="text-input tyre-text-input" value="${val}" step="${currentStep}"${currentMin}${currentMax}>
